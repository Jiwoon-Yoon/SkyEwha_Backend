...
def recommend_titles(db: Session, video_id: int) -> list[str]:
    return generate_title_from_keywords(db, video_id)

def recommend_hashtags(db: Session, video_id: int, top_n: int = 10) -> list[str]:
    return recommend_hashtags_from_keywords(db, video_id, top_n=top_n)

def recommend_similar_videos(db: Session, video_id: int, limit: int = 10) -> list[YoutubeTitleResponse]:
